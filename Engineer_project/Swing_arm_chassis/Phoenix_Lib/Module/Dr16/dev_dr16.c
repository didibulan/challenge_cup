...
static void Dr16_Receive(UartInstance_s *uart_instance) {
    ...
    //接收数据校验
    if(uart_instance->rx_size != REMOTE_CONTROL_FRAME_SIZE){
        memset(&dr16_instance->uart_instance->rx_buff,0,sizeof(dr16_instance->uart_instance->rx_buff));
        memset(&dr16_instance->dr16_handle,0,sizeof(dr16_instance->dr16_handle));
        memset(&dr16_instance->dr16_mouse,0,sizeof(dr16_instance->dr16_mouse));
        memset(&dr16_instance->dr16_keyboard,0,sizeof(dr16_instance->dr16_keyboard));
    } else {
    ...
    HAL_UARTEx_ReceiveToIdle_DMA(dr16_instance->uart_instance->uart_handle,
                             dr16_instance->uart_instance->rx_buff,
                             sizeof(dr16_instance->uart_instance->rx_buff));
}
