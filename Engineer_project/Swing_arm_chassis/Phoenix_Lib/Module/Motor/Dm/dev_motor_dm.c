...
DmMotorInstance_s *Motor_DM_Register(DmMotorInitConfig_s *config){
    ...
    motor_instance->motor_state = DM_DISABLE;
    ...
    motor_instance->parameters.kp_int = config->parameters.kp_int;
    motor_instance->parameters.kd_int = config->parameters.kd_int;
...
bool Motor_Dm_FullParam_MIT_Control(
        const DmMotorInstance_s *motor,
        const float pos,
        const float vel,
        const float kp,
        const float kd,
    ...
    const uint16_t pos_tmp = float_to_uint(pos, -motor->parameters.pos_max, motor->parameters.pos_max, 16);
    const uint16_t vel_tmp = float_to_uint(vel, -motor->parameters.vel_max, motor->parameters.vel_max, 12);
    const uint16_t kp_tmp = float_to_uint(kp, 0, motor->parameters.kp_max, 12);
    const uint16_t kd_tmp = float_to_uint(kd, 0, motor->parameters.kd_max, 12);
    const uint16_t tor_tmp = float_to_uint(tor, -motor->parameters.tor_max, motor->parameters.tor_max, 12);
...
bool Motor_Dm_Transmit(const DmMotorInstance_s *motor){
    ...
    #ifdef USER_CAN_FD
        if (motor->control_mode == DM_POSVEL) {
            motor->can_instance->tx_conf.Identifier += 0x100;
            uint8_t flag = Can_Transmit(motor->can_instance);
            motor->can_instance->tx_conf.Identifier -= 0x100;
            return flag;
        }
    #endif
