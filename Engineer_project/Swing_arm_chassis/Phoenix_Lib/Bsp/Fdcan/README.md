...
## 注意事项
- 使用FIFO1在中断配置中勾选FDCANx interrupt 0 和 FDCANx interrupt 1
...
1. 在CubeMx中进行以下配置：
   ![](../../pic/6.png)
2. 在config结构体中额外定义以下两个值：mode决定是标准can还是可变波特率的fdcan(和CubeMX的一致)，DLC决定数据域的长度，**请务必使用HAL的宏定义！！**
```C
CanInitConfig_s fdcan1_config = {
