...
static void Motor_Lk_Decode(CanInstance_s *can_instance){
    ...
    LkMotorInstance_s *motor = (LkMotorInstance_s *)can_instance->parent_ptr;
    switch(can_instance->rx_buff[0]){
        case 0xA1: // 广播模式反馈数据
        motor->message.rotor_last_position = motor->message.rotor_position;
        motor->message.motor_temperature = can_instance->rx_buff[1];
        motor->message.torque_current = (int16_t)(can_instance->rx_buff[3] << 8 | can_instance->rx_buff[2]);
        motor->message.rotor_velocity = (int16_t)(can_instance->rx_buff[5] << 8 | can_instance->rx_buff[4]);
        motor->message.rotor_position = (int16_t)(can_instance->rx_buff[7] << 8 | can_instance->rx_buff[6]);
        break;
        default:
        break;
    }
...
LkMotorInstance_s *Motor_Lk_Register(LkMotorInitConfig_s *config) {
    ...
    // 分配内存
    LkMotorInstance_s *motor = (LkMotorInstance_s *)user_malloc(sizeof(LkMotorInstance_s));
...
bool Motor_Lk_Control(LkMotorInstance_s *motor, float target){
    ...
    if(motor->control_mode == LK_VELOCITY){
        ...
        motor->output = Pid_Calculate(motor->velocity_pid, motor->target_velocity, motor->message.rotor_velocity);//目前是编码器
    }else if(motor->control_mode == LK_POSITION){
        motor->target_position = target;
        motor->target_velocity = Pid_Calculate(motor->angle_pid, motor->target_position, motor->message.rotor_position);//目前是编码器
        motor->output = Pid_Calculate(motor->velocity_pid, motor->target_velocity, motor->message.rotor_velocity);//目前是编码器
    }else{
        return false;
    }
